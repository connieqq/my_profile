...
- 右侧文字区保持紧凑排布，使用居中对齐和较小 padding，跳转按钮按自然间距跟随内容，不使用 `margin-top: auto` 推到底部。
...
- 图书封面使用区别于普通卡片的封面视觉，`.book-cover::after` 输出 `> 点击展开` 入口；该入口位于封面内部右侧、垂直居中，使用圆角胶囊按钮样式。
...
- AI Cookbook 图书封面内的展开入口应保持在封面内部右侧居中；窄屏可回退到封面右上角，避免遮挡封面文字或造成横向溢出。
...
- `button.book-cover` 的 `::after` 伪元素承担封面内 `> 点击展开` 的视觉提示，桌面端定位在 `top: 50%`、右侧内边距附近；移动端可定位到右上角并保持在封面内部。
...
- AI Cookbook 图书封面内展示 `> 点击展开` 圆角按钮，位于封面右侧居中，且不遮挡封面标题、描述和底部信息。
