...
- 图片加载优先使用同名 `.webp` 资源，原 JPG/PNG 保留作为兼容兜底；不因生成 WebP 而删除原图。
...
- 4 个文档右页图片需同步维护同名 `.webp` 版本；如果替换任意 PNG 原图，必须重新生成对应 WebP，并同步更新 HTML 中的图片尺寸属性。
- 切换不同文档进行图书翻页时，展开态图书区域高度必须保持一致；流程图在右页固定区域内等比缩放适配，不允许因图片宽高比不同撑高整本书。
- 展开态图书区域高度尽量不超过 AI Cookbook 模块原本的视觉高度，避免展开后大幅增加页面纵向占用。
...
- 当前站点通过静态资源部署，图片性能优先通过本地生成 WebP、响应式加载和懒加载解决，不依赖 Cloudflare/Vercel 配置改动。
- `assets/` 下的 JPG/PNG 原图保留作为 fallback 和后续素材维护来源；它们会占用仓库/部署包空间，但只要页面优先引用 WebP，现代浏览器不会同时请求 fallback 原图，因此不会直接拖慢页面加载。
- 后续替换图片时，应同步更新同名 `.webp` 资源；首屏图片需关注体积和展示尺寸，非首屏图片保持懒加载。
